...
# 사용자 입력을 통해 옵션 선택
print("PPT 보고서 옵션을 선택해주세요:\n")

# 글자 크기 선택
print("1. 사이즈 8")
print("2. 사이즈 9")
print("3. 사이즈 10")
font_size_option = int(input("글자 사이즈를 선택하세요 : "))

font_sizes = {1: Pt(8), 2: Pt(9), 3: Pt(10)}
selected_font_size = font_sizes.get(font_size_option, Pt(8))

# 테이블 정렬 기준 선택
print("\n1. 좌측 정렬")
print("2. 가운데 정렬")
print("3. 우측 정렬")
alignment_option = int(input("글자 정렬 기준을 선택하세요 : "))

alignments = {1: PP_ALIGN.LEFT, 2: PP_ALIGN.CENTER, 3: PP_ALIGN.RIGHT}
selected_alignment = alignments.get(alignment_option, PP_ALIGN.LEFT)

# 보고서 모드 선택
print("\n1. 라이트모드")
print("2. 다크모드")
print("3. 여름휴가 모드")
mode_option = int(input("보고서 모드를 선택하세요 : "))

modes = {
    1: {'background': RGBColor(255, 255, 255), 'text': RGBColor(0, 0, 0)},  # 라이트모드
    2: {'background': RGBColor(0, 0, 0), 'text': RGBColor(255, 255, 255)},  # 다크모드
    3: {'background': RGBColor(173, 216, 230), 'text': RGBColor(0, 0, 0)}  # 다른 모드
}
selected_mode = modes.get(mode_option, modes[1])
        ...
        for cell in (cell_0, cell_1):
            for paragraph in cell.text_frame.paragraphs:
                for run in paragraph.runs:
                    run.font.size = selected_font_size  # 선택한 글자 크기 적용
                    run.font.name = 'Arial'
