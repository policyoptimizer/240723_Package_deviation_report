...
function generateImage(csvText, ctx, callback) {
    ...
    let rowHeight = 50;
    let canvasHeight = calculateCanvasHeight(data, rowHeight);
    let colWidths = calculateColumnWidths(headers, data, ctx); // 열 너비를 적절히 조정
...
function calculateCanvasHeight(data, rowHeight) {
    // 전체 캔버스 높이를 데이터 양에 따라 계산
    let totalHeight = rowHeight * (data.length + 1) + 50;
    return totalHeight;
}

function calculateColumnWidths(headers, data, ctx) {
    let colWidths = headers.map(() => 150); // 기본 열 너비

    // 열 너비를 데이터와 헤더에 따라 조정
    headers.forEach((header, i) => {
        let maxWidth = ctx.measureText(header).width + 20; // 헤더의 너비
        data.forEach(row => {
            let cellWidth = ctx.measureText(row[i] || "").width + 20;
